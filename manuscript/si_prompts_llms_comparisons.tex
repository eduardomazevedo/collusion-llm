...
We also compare LLM responses using different models from OpenAI that are suitable for our task. In Table~\ref{tab:si_model_benchmark_first_run} we compute for each model the same metrics we did for the prompt comparison, using the first LLM response for each transcript-model pair, and keeping the \texttt{SimpleCapacityV8.1.1} prompt fixed. Since some of the older models do not allow imposing a specific schema for the LLM response, we use a version of this prompt that simply adds to the original a concise description of the desired schema, following the guidance of OpenAI.
